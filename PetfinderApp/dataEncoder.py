...
class encoder:
	...
	def encodeAnimal(self,breeds, colors, states):
		...
		for row in self.data["animals"]:
			...
			endDate = row["status_changed_at"]
			adoptionSpeed = abs((row["published_at"] - row["status_changed_at"]).days)




			print(f"Published at : {row['published_at']}")
			print(f"Status changed at : {row['status_changed_at']}")
			print(f"Adoption speed: {adoptionSpeed}" )	
			print("")
			print("")
